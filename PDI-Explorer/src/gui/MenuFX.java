...
public class MenuFX extends Application {
	...
	private SpinnerValueFactory<Integer> valueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(3, 6,
			initialNb);
	...
	private void init(Stage stage) throws FileNotFoundException {
		...
		hBox.setPadding(new Insets(primaryScreenBounds.getHeight() / 12, primaryScreenBounds.getWidth() / 15,
				primaryScreenBounds.getHeight() / 15, primaryScreenBounds.getWidth() / 4.7));
	...
	private void simLaunch() {
		stratChoice.setOnAction((event) -> {
			...
			if (stratNbr > 0) {
				@SuppressWarnings("unused")
				SimulationFX sim = new SimulationFX(primaryStage, startButton, enveloppe, valueFactory,
						EXPLORATION_STRATEGY, items, primaryScreenBounds, views, stratNbr);
			}
		});
	}
